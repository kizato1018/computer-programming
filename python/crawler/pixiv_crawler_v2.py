...
class Worker(threading.Thread):
    def __init__(self, queue, num, url, stop):
    ...
    def run(self):
        def ok_check(bookmarkCount, viewCount):
            return (bookmarkCount > 1000 and bookmarkCount * 15 > viewCount) or bookmarkCount > 3000
        ...
        while self.queue.qsize() > 0:
            ...
            tags = item['tags']['tags']
            with open("./log/log.txt", 'ab') as fp:
                    fp.write((log+'\n').encode('utf8'))
            if(bookmarkCount >= 1000 and bookmarkCount * 15 < viewCount):
                with open("./log/to_check", 'ab') as fp:
                    fp.write(('https://www.pixiv.net/artworks/'+log+'\n').encode('utf8'))
            ...
            if img_res_code == 200:                 # 如果請求成功
                ...
                with open(path+img_save_name, 'wb') as fp:
