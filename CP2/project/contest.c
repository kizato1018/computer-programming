    ...
    for( int32_t i = 0 ; i < NUMBER - 1 ; i++ )
    {
        for( int32_t j = i + 1 ; j < NUMBER ; j++ )
        {
            ...
            for( int32_t k = 0 ; k < 1000 ; k++ )
            {
                ...
                {
                	int32_t value[2];
					value[0] = player[i].pick( table , player[i].score[0] , last1 ); 
            		value[1] = player[j].pick( table , player[j].score[1] , last0 );
					...
					if(value[0] < value[1])
					{
						...
						{
							int32_t a = palyer[i].choose( table , value[0]);	//玩家選擇一個牌堆拿走 
							for( int32_t x = 0 ; x < flag[a] ; x++)				//計算此牌堆牛頭數量 
								player[i].score += count( table[a][x]);
							memset(table[a] , 0 , 5);							//reset牌堆 
						...
						{
							int32_t a = palyer[j].choose( table , value[1]);	//玩家選擇一個牌堆拿走 
							for( int32_t x = 0 ; x < flag[a] ; x++)				//計算此牌堆牛頭數量
